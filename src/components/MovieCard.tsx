...
export default function MovieCard({
  ...
  const averageColor = useImageAverageColor(posterUrl, title);

  return (
    <motion.div
      className="relative flex-none w-[150px] sm:w-[170px] md:w-[190px] aspect-[2/3] select-none group cursor-pointer"
    ...
    >
      {/* Dynamic Glow using the calculated dynamic average color */}
      <div
        className="absolute -inset-1.5 sm:-inset-2 rounded-xl opacity-20 blur-xl group-hover:opacity-80 group-hover:blur-2xl transition-all duration-500 pointer-events-none -z-10"
        style={{ backgroundColor: averageColor }}
      />

      {/* Main card contents container with rounded corner and crop */}
      <div className="relative w-full h-full rounded-xl overflow-hidden bg-[#111111]/92 border border-white/10 shadow-xl">
        {/* Poster Image */}
        <img
          src={posterUrl}
          alt={title}
